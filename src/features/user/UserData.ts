...
export class UserData {
	...
	public static async EmbedCahceRegister(threadId: string): Promise<boolean> {
		...
		if (thread) return UserData._EmbedCahceRegister(thread);
		else return false;
	}

	public static async _EmbedCahceRegister(
	...
	public normalization() {
		this.historyThreadIds = this.historyThreadIds.filter(
			(id) => UserData.getEmbedJsonFromThreadId(id) !== undefined,
		);
	}
